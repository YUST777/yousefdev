...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const toggleMenu = useCallback(() => {
    ...
    // Use openRef for synchronous toggle logic to avoid stale state issues
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#000000', '#ffffff'];
          let arr = [...raw];
          if (arr.length >= 3) {
            ...
            {items && items.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  <a className="sm-panel-item" href={it.link} aria-label={it.ariaLabel || it.label} data-index={idx + 1} onClick={closeMenu}>
